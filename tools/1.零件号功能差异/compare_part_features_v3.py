...
def compare_part_features_v3(master_file, diff_file, output_file):
    """
    比对零件号之间的功能差异（基于●标记）
    输出格式与技术差异输入文件一致（5列）
    ...
    for i, col in enumerate(master_df.columns):
        if pd.notna(col) and '标配' in str(col):
            ...
            print(f"\n找到'标配'列在索引 {standard_column_index}: {col}")
            break
    
    if standard_column_index is None:
        print("\n警告：未找到'标配'列，使用默认索引244")
        standard_column_index = 243
    
    # 获取功能列（从"标配"列之后开始）
    feature_columns = master_df.columns[standard_column_index + 1:].tolist()
    print(f"从'标配'列之后（索引 {standard_column_index + 1}）识别到 {len(feature_columns)} 个功能/配置列")
...
if __name__ == "__main__":
    ...
    # 执行比对
    compare_part_features_v3(master_file, diff_file, output_file)
