...
def compare_part_features_final(master_file, diff_file, output_file):
    ...
    for i, value in enumerate(second_row):
        if pd.notna(value) and '标配' in str(value):
            ...
            print(f"\n找到'标配'单元格在第二行索引 {standard_column_index}")
            break
    
    if standard_column_index is None:
        print("\n警告：未在第二行找到'标配'单元格，使用默认索引243")
        standard_column_index = 243
    ...
    for col in feature_columns:
        value = second_row[col]
        if pd.notna(value):
            feature_values[col] = str(value).strip()
    
    print(f"\n功能列的值（从第二行获取，共 {len(feature_values)} 个）:")
    ...
    for _, diff_row in diff_df.iterrows():
        ...
        added_features_str = '●'.join(sorted([feature_values.get(col, col) for col in added_features])) if added_features else ''
        cancelled_features_str = '●'.join(sorted([feature_values.get(col, col) for col in cancelled_features])) if cancelled_features else ''
