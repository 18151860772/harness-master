...
- 每个Family在每个车型中生成一个零件号，零件号规则是Family + 车型编号（从001开始）
- 例如：FLOOR001、FLOOR002...（FLOOR在车型1中生成FLOOR001，在车型2中生成FLOOR002）
- 根据配置表中该Family在各版型的打点情况，用X标记
- 特征值代码用●标记
- 相同Family名称在不同车型中生成不同的零件号
...
### 4. 版型打点匹配
- 根据配置表中每个Family在各版型的打点情况
- 如果该Family在某个版型中打点，则对应的零件号在该版型中打X
...
工具会为每个Family生成一个零件号，并根据配置表中该Family在各版型的打点情况，用X标记。

**示例：FLOOR Family**

假设FLOOR涉及以下Option：EP01, KG01, KQ00, KB02, ALL, JL04, RJ01, KP02, LHD, FW98...

工具会生成零件号：FLOOR001，并在各版型中根据打点情况标记X：

- **版型1**：FLOOR001 → X（该Family在此版型中打点）
- **版型2**：FLOOR001 → X（该Family在此版型中打点）
- **版型3**：FLOOR001 → （该Family在此版型中不打点）
- ...

每个Family只生成一个零件号，打点关系与配置表中版型与Option打点完全一致。

### 零件号编号规则

- 每个Family在每个车型中生成一个零件号
- 零件号格式：Family + 车型编号（从001开始）
- 例如：FLOOR001、FLOOR002、IP001、IP002...
- 相同Family名称在不同车型中生成不同的零件号
...
双击打开 `masterlist-generator-new.html` 文件，在浏览器中运行。
...
- 每个Family在每个车型中生成一个零件号（Family + 车型编号）
- 相同Family名称在不同车型中生成不同的零件号
